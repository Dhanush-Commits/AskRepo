...
st.set_page_config(page_title='AskRepo | Github repo AI', page_icon="")
...
with st.sidebar:
    ...
    if st.button(st.session_state['button_msg']):
        ...
        if is_valid_repolink(repolink):
            ...
            st.session_state['title'] = f"Chat with {reponame}(using CodeLlama-7B-GGUF)"
...
if prompt := st.chat_input(""):
    ...
    with st.chat_message("assistant"):
        try:
            ...
            response = st.write_stream('Sorry, Llama categorized your question as unsafe. Try another repo or question')
